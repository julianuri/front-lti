...
const MemoryForm = ({ items, setItems, setShowModal }: MemoryFormProps) => {
  ...
  const onSubmit = (data: any) => {
      const newData = {
        ...
        secondMatch: (answerType === MemoryAnswerType.TEXT) ? data.Answer : data.Answer[0],
        fileName: (answerType === MemoryAnswerType.IMAGE) ? data.Answer[0].name : null,
        type: answerType,
      };
  ...
  const onFileChange = function(e) {
    if (e.target?.files && !validFileSize(e.target.files[0])) {
      toast.error('Image should be max 1MB');
      e.target.value = '';
    }
  };
      ...
      <form className={styles.form} onSubmit={handleSubmit(onSubmit)}>
        ...
        <div className={styles.fullRow}>
          ...
          <div>
            <input
              type='text'
              className={(errors['Concept'] != null) ? styles.isInvalidField : undefined} {...register('Concept', { required: true })}
            />
          </div>
        </div>
          ...
          (answerType === MemoryAnswerType.TEXT ) ?
            (<div className={styles.fullRow}>
              ...
              <div>
                <input
                  type='text'
                  className={(errors['Answer'] != null) ? styles.isInvalidField : undefined} {...register('Answer', { required: true })}
                />
              </div>
            </div>) :
            ...
            (<div className={styles.fullRow}>
              ...
              <div>
                <input
                  type='file'
                  accept="image/*"
                  className={(errors['Answer'] != null) ? styles.isInvalidField : undefined} {...register('Answer', {
